...
//** Inicializar la datatable como tal y dejar a esta variable como la encargada de 
//** listar a los usuarios, de hacer la petición de fetch, de convertir los datos de json y de recorrer todo.
...
const listVentas= async() => {
    try {
        ...
        ventas.forEach((venta, index) => {
            console.log(venta);
            let fecha = new Date(venta.DAT_CEXP.date);
            let dia = fecha.getDate();
            let mes = fecha.getMonth();
            let ano = fecha.getFullYear();
            let fechaEnString = `${dia}/${mes+1}/${ano}`;
            console.log(venta.DAT_CEXP);
            content +=`
                <tr>
                    <td>${index+1}</td>  
                    <td>${venta.DAT_CNAME}</td>  
                    <td>${venta.DAT_CCOMPANY}</td>  
                    <td>${venta.DAT_CPLAN}</td>  
                    <td>${venta.DAT_CPIN}</td>
                    <td>${fechaEnString}</td>
                    <td>${venta.DAT_CPO}</td>
                    <td>${venta.DAT_CORDER}</td>
